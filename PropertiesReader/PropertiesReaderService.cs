...
namespace PropertiesReader
{
    public class PropertiesReaderService : IDisposable
    {
        ...
        public DataTable ReadProperties(string assmFilePath)
        {
            ...
            DataTable prpsTable = new DataTable();
            _ = prpsTable.Columns.Add(FILE_PATH_COLUMN_NAME);

            IXAssembly assm = (IXAssembly)m_App.Documents.Open(assmFilePath, Xarial.XCad.Documents.Enums.DocumentState_e.ReadOnly);

            IXDocument3D[] refDocs = assm.Configurations.Active.Components.Flatten()
                .GroupBy(x =>
                {
                    try
                    {
            ...
            {
                DataRow row = prpsTable.NewRow();
                row[FILE_PATH_COLUMN_NAME] = refDoc.Path;

                foreach (Xarial.XCad.Data.IXProperty prp in refDoc.Properties)
                {
